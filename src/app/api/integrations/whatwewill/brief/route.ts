...
import { generateCareerBrief } from "@/lib/integrations/whatwewill-brief";
import {
...
export async function POST(req: NextRequest) {
  ...
  const validated = validateBriefPayload(rawBody);
  if (!validated.ok) {
    return NextResponse.json<IntegrationErrorResponse>(
  ...
  try {
    const brief = await generateCareerBrief(validated.payload);
    return NextResponse.json(brief, { status: 200 });
  } catch (error) {
    console.error("WhatWeWill brief API error", error);
    return NextResponse.json<IntegrationErrorResponse>(
      {
        code: "INTERNAL_ERROR",
...
function validateBriefPayload(
  input: unknown
): { ok: true; payload: WhatWeWillBriefRequest } | { ok: false; message: string } {
  ...
  const tone = asOptionalString(body.tone) as "supportive" | "direct" | "coach" | undefined;
  const maxWords = typeof body.maxWords === "number" ? body.maxWords : undefined;
  const includeIllustrativeLinks =
    typeof body.includeIllustrativeLinks === "boolean"
      ? body.includeIllustrativeLinks
      : undefined;
  ...
  if (tone && !["supportive", "direct", "coach"].includes(tone)) {
    return { ok: false, message: "tone must be one of supportive, direct, or coach" };
  }
